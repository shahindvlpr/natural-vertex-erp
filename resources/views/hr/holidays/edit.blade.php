...
@section('title', 'Edit Holiday - Natural Vertex ERP')
...
<div class="form-card">
    <div class="form-card-header">
        <h5 style="margin:0; font-size:16px; font-weight:600;">
            <i class="fas fa-edit" style="color:#6c5ce7;"></i> Edit Holiday: {{ $holiday->name }}
        </h5>
    </div>
    ...
    <div class="form-card-body">
        ...
        <form action="{{ route('hr.holidays.update', $holiday->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label class="form-label">Holiday Name <span class="required">*</span></label>
                <input type="text" name="name" value="{{ old('name', $holiday->name) }}" class="form-control" required>
            </div>

            <div class="form-group">
                <label class="form-label">Date <span class="required">*</span></label>
                <input type="date" name="date" value="{{ old('date', $holiday->date->format('Y-m-d')) }}" class="form-control" required>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="description" class="form-control" rows="2">{{ old('description', $holiday->description) }}</textarea>
            </div>

            <div class="form-group">
                <label style="display:flex; align-items:center; gap:8px; font-size:13px; cursor:pointer;">
                    <input type="checkbox" name="is_paid" {{ old('is_paid', $holiday->is_paid) ? 'checked' : '' }}>
                    <span style="font-weight:600; color:#4a4a5a;">Paid Holiday</span>
                </label>
            </div>

            <div class="form-group">
                <label style="display:flex; align-items:center; gap:8px; font-size:13px; cursor:pointer;">
                    <input type="checkbox" name="is_active" {{ old('is_active', $holiday->is_active) ? 'checked' : '' }}>
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('hr.holidays.index') }}" class="btn-cancel">
