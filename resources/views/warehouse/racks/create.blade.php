    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h5 class="mb-0 fw-bold text-dark">
                <i class="fas fa-plus-circle text-primary me-2"></i> Add New Rack / Shelf
            </h5>
            ...
            <form action="{{ route('warehouse.racks.store') }}" method="POST">
                ...
                <div class="px-4 py-4">
                    <div class="row g-4">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control bg-light border-0 py-2 @error('name') is-invalid @enderror" placeholder="E.g. Rack A" required>
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-3">
                            <label for="code" class="form-label fw-bold text-muted small text-uppercase">Code</label>
                            <input type="text" name="code" id="code" value="{{ old('code') }}" class="form-control bg-light border-0 py-2 @error('code') is-invalid @enderror" placeholder="E.g. A-1">
                            @error('code') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-3">
                            <label for="description" class="form-label fw-bold text-muted small text-uppercase">Description</label>
                            <input type="text" name="description" id="description" value="{{ old('description') }}" class="form-control bg-light border-0 py-2 @error('description') is-invalid @enderror" placeholder="Short description">
                ...
                <div class="px-4 py-3 border-top bg-light d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary px-4 shadow-sm hover-lift">
                        <i class="fas fa-save me-1"></i> Save Rack
                    </button>
                </div>
            </form>
        </div>
