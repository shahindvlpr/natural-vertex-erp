    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h5 class="mb-0 fw-bold text-dark">
                <i class="fas fa-exchange-alt text-primary me-2"></i> Initiate Stock Transfer
            </h5>
        ...
        <div class="card-body p-0">
            <form action="{{ route('warehouse.transfers.store') }}" method="POST">
                ...
                <div class="px-4 py-4">
                    <div class="row g-4">
                        <div class="col-md-6">
                            <label for="from_warehouse_id" class="form-label fw-bold text-muted small text-uppercase">From Warehouse</label>
                            <select name="from_warehouse_id" id="from_warehouse_id" class="form-select bg-light border-0 py-2 @error('from_warehouse_id') is-invalid @enderror">
                        ...
                        <div class="col-md-6">
                            <label for="to_warehouse_id" class="form-label fw-bold text-muted small text-uppercase">To Warehouse <span class="text-danger">*</span></label>
                            <select name="to_warehouse_id" id="to_warehouse_id" class="form-select bg-light border-0 py-2 @error('to_warehouse_id') is-invalid @enderror" required>
                                <option value="">Select Warehouse</option>
                                @foreach($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}">{{ $warehouse->name }}</option>
                                @endforeach
                        ...
                        <div class="col-md-6">
                            <label for="transfer_date" class="form-label fw-bold text-muted small text-uppercase">Transfer Date <span class="text-danger">*</span></label>
                            <input type="date" name="transfer_date" id="transfer_date" value="{{ old('transfer_date', date('Y-m-d')) }}" class="form-control bg-light border-0 py-2 @error('transfer_date') is-invalid @enderror" required>
                            @error('transfer_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-6">
                            <label for="notes" class="form-label fw-bold text-muted small text-uppercase">Notes</label>
                            <input type="text" name="notes" id="notes" value="{{ old('notes') }}" class="form-control bg-light border-0 py-2" placeholder="Optional notes">
                        </div>
                    </div>
                </div>
                ...
                <div class="px-4 py-3 border-top bg-light d-flex justify-content-end gap-2">
                    <a href="{{ route('warehouse.transfers.index') }}" class="btn btn-light px-4">Cancel</a>
                    <button type="submit" class="btn btn-primary px-4 shadow-sm hover-lift">
                        <i class="fas fa-save me-1"></i> Initiate Transfer
                    </button>
                </div>
            </form>
        </div>
